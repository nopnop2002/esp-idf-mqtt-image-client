...
makeSendText(out, "TOPIC", topic, fsize, imagetype);
makeSendText(out, "SHOW", "", "", "");
*/
...
void app_main() {
	...
	ESP_ERROR_CHECK(mountSPIFFS(MOUNT_POINT, "storage", 7));
	...
	ws_server_start();
	xTaskCreate(&server_task, "server_task", 1024*2, (void *)cparam0, 6, NULL);
	...
	xTaskCreate(converte, "converte_task", 1024*4, NULL, 4, NULL);
	...
	while(1) {
		...
		if (cJSON_GetObjectItem(root, "id")) {
			...
			if ( strcmp (id, "subscribe-data") == 0) {
				// receive base64 file name from converte
				char *topic = cJSON_GetObjectItem(root,"topic")->valuestring;
				char *imagetype = cJSON_GetObjectItem(root,"imagetype")->valuestring;
				char *imagefile = cJSON_GetObjectItem(root,"imagefile")->valuestring; // base64 encorded image
				ESP_LOGI(TAG, "topic=[%s]",topic);
				ESP_LOGI(TAG, "imagetype=[%s]",imagetype);
				ESP_LOGI(TAG, "imagefile=[%s]",imagefile);

				// verify base64 file size
				FILE* file = fopen(imagefile, "rb");
				if (file == NULL) {
					ESP_LOGE(TAG, "Failed to open file for base64 file");
					continue;
				}
				struct stat st;
				fstat(fileno(file), &st);
				ESP_LOGI(TAG, "%s st.st_size=%ld", imagefile, st.st_size);
				fclose(file);
				file = NULL;
				...
				fread(base64_buffer, st.st_size, 1, file);
				fclose(file);
				unlink(imagefile);
				ESP_LOGI(TAG, "base64 file unlink");
				...
				while(1) {
					...
					ws_server_send_text_all(out,len);
					imageIndex = imageIndex + imageLen;
					if (imageIndex == st.st_size) break;
				}
				len = makeSendText(out, "SHOW", "", "", "");
				ws_server_send_text_all(out,len);
