...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
	...
	switch (event->event_id) {
		...
		case MQTT_EVENT_DATA:
			...
			ESP_LOGD(TAG, "event->topic_len=%d event->data_len=%d", event->topic_len, event->data_len);
			ESP_LOGD(TAG, "TOPIC=[%.*s]", event->topic_len, event->topic);
			ESP_LOG_BUFFER_HEXDUMP(TAG, event->data, 10, ESP_LOG_DEBUG);

			MQTT_t mqttBuf;
			mqttBuf.topic_type = SUBSCRIBE;
			if (event->topic_len != 0) sequence = 0;
			ESP_LOGI(TAG, "sequence=%d", sequence);
			...
			//xQueueSendFromISR(xQueueSubscribe, &mqttBuf, &xHigherPriorityTaskWoken);

#if 0
			char * topic;
			topic = malloc(event->topic_len+1);
			if (topic == NULL) {
				ESP_LOGE(TAG, "topic malloc fail");
			...
			if (payload == NULL) {
				ESP_LOGE(TAG, "payload malloc fail");
				while(1) { vTaskDelay(1); }
			}
			...
			ESP_LOGI(TAG, "strlen(my_json_string)=%d",strlen(my_json_string));
